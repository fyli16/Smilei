...
// Constructor
// ---------------------------------------------------------------------------------------------------------------------
ElectroMagnBC1D_Damping::ElectroMagnBC1D_Damping( Params &params, Patch *patch, unsigned int i_boundary )
...
// ---------------------------------------------------------------------------------------------------------------------
double ElectroMagnBC1D_Damping::dampingCoefficient( double x ) const
{
...
// ---------------------------------------------------------------------------------------------------------------------
void ElectroMagnBC1D_Damping::apply( ElectroMagn *EMfields, double, Patch *patch )
{
    // Physical position (along x) of a node, given its local index and grid staggering.
    // shift = 0.0 for a primal node, 0.5 for a dual node.
    ...
    Field1D *Ey1D = static_cast<Field1D *>( EMfields->Ey_ ); // primal in x
    Field1D *Ez1D = static_cast<Field1D *>( EMfields->Ez_ ); // primal in x
    Field1D *By1D = static_cast<Field1D *>( EMfields->By_ ); // dual   in x
    Field1D *Bz1D = static_cast<Field1D *>( EMfields->Bz_ ); // dual   in x

    // Primal-in-x fields: Ey, Ez
